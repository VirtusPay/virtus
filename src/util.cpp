...
const char * const BITCOIN_CONF_FILENAME = "virtus.conf";
const char * const BITCOIN_PID_FILENAME = "virtusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "virtus" is a composite category enabling all Virtus-related debug output
            if(ptrCategory->count(string("virtus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VirtusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VirtusCore
    // Mac: ~/Library/Application Support/VirtusCore
    // Unix: ~/.virtuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VirtusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VirtusCore";
#else
    // Unix
    return pathRet / ".virtuscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty virtus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override virtus.conf
