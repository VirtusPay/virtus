...
// ***TODO*** ScanHash is not yet used in Virtus
//
...
{
    LogPrintf("VirtusMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("virtus-miner");
    ...
    try {
        ...
        while (true) {
            ...
            */
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("VirtusMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("VirtusMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("VirtusMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("VirtusMiner -- runtime error: %s\n", e.what());
        return;
    }
}
