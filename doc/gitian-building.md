...
*Setup instructions for a Gitian build of Virtus Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Virtus
...
- [Building Virtus Core](#building-virtus-core)
...
Clone the git repositories for Virtus Core and Gitian.
...
in the Virtus Core repository under 'Fetch and build inputs' to install sources which require
...
To build Virtus Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Virtus Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/virtus/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/virtuspay/virtus
...
For example:
```bash
URL=https://github.com/crowning-/virtus.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit virtus=${COMMIT} --url virtus=${URL} ../virtus/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit virtus=${COMMIT} --url virtus=${URL} ../virtus/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit virtus=${COMMIT} --url virtus=${URL} ../virtus/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the virtus git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../virtus/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/virtus.git
SIGPATH=/some/root/path/virtus-detached-sigs.git

./bin/gbuild --url virtus=${BTCPATH},signature=${SIGPATH} ../virtus/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/virtus-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/virtus-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/virtus-osx-build.assert
```
...
[virtus/gitian.sigs](https://github.com/virtuspay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
